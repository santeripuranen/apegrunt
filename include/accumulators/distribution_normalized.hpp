/** @file distribution_normalized.hpp
	...
	$Id: $
*/
#ifndef APEGRUNT_ACCUMULATORS_DISTRIBUTION_NORMALIZED_HPP
#define APEGRUNT_ACCUMULATORS_DISTRIBUTION_NORMALIZED_HPP
...
std::vector< distribution_bin<Sample> > distribution_normalized( boost::accumulators::accumulator_set< Sample, Args... >& acc )
{

	using value_type = distribution_bin<Sample>;
	using result_type = std::vector< bin_type >;
	...
	const auto diststd = std(acc);
	const auto distcount = count(acc);

	std::transform( cbegin(d), cend(d), begin(bins),
...
#endif // APEGRUNT_ACCUMULATORS_DISTRIBUTION_NORMALIZED_HPP
